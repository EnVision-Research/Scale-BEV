...
boxes = mmcv.load('./work_dirs/llyft_onlycar_cbgs/latest.pkl')
...
for ann, label, score in zip(ann_infos, ann_label, ann_scores):
    ...
    # lidar2ego
    # box.translate(-np.array(infos[index]['lidar2ego_translation']))
    # box.rotate(Quaternion(infos[index]['lidar2ego_rotation']).inverse)
    # 从车身坐标系->相机坐标系
...
cv2.imwrite('/mnt/cfs/algorithm/hao.lu/Code/BEVDet/scripts/Vis/lyft2lyft.png', img)
