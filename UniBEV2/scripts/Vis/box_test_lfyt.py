...
boxes = mmcv.load('/mnt/cfs/algorithm/hao.lu/Code/BEVDet/work_dirs/lyft_onlycar_cbgs_nu/latest.pkl')
...
index = 0
CAM_index = 0
ann_infos = boxes[index]['pts_bbox']['boxes_3d']
ann_label = boxes[index]['pts_bbox']['labels_3d']
cam_infos = infos[index]['cams'][cam_names[CAM_index]]
img_path = infos[index]['cams'][cam_names[CAM_index]]['data_path']
img = cv2.imread(img_path)


for ann, label in zip(ann_infos, ann_label):
    # box information
    # ann = ann_infos[0]
    ann = np.array(ann)
...
cv2.imwrite('/mnt/cfs/algorithm/hao.lu/Code/BEVDet/scripts/Vis/lyft_img_test.png', img)
