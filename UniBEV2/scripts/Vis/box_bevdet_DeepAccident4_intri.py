...
size_decrease = (int(width/1.5), int(height/2))
img = cv2.resize(img,size_decrease,interpolation=cv2.INTER_CUBIC)
# img = cv2.flip(img, 1)
# infos[index]['gt_boxes']
# infos[index]['gt_names']
print(cam_infos['camera_intrinsic_matrix'])
...
sensor2ego_rotation = [ego2im_Q.x.astype(float), ego2im_Q.y.astype(float), ego2im_Q.z.astype(float), ego2im_Q.w.astype(float)]
...
#            [0.9999118, 0.01015593, 0.00855874]])
    ...
    camera_intrinsic = [[1142.5184053936916/1.5, 0.0, 800.0/1.5],
                        [0.0, 1142.5184053936916/2, 450.0/2],
                         [0.0, 0.0, 1.0]]
