...
# rot_axis
R = np.dot(infos[index]['ego_to_world_matrix'], infos[index]['lidar_to_ego_matrix'])
sy = math.sqrt(R[0, 0] * R[0, 0] + R[1, 0] * R[1, 0])
lidar_pitch_radian = math.atan2(-R[2, 0], sy)

lidar2cam_rt = cam_infos['lidar_to_camera_matrix']
ego2lidar_rt = np.linalg.inv(infos[index]['lidar_to_ego_matrix'])
...
# lidar_to_camera_matrix = ego2cam_rt[0:3, [1, 0, 2]]
temp = np.array(cam_infos['lidar_to_camera_matrix'][0:3, 3][[1, 2, 0]])
temp[1] = -1.0 * temp[1]
ego2cam[0:3, 3][[1, 2, 0]]
view = np.eye(4)
view[:3, :3] = np.array(lidar_to_camera_matrix)
view[:3, -1] = temp # ego2cam[0:3, 3][[1, 2, 0]] # temp
ego2img = np.linalg.inv(view)
ego2im_Q = Quaternion(matrix=np.array(ego2img[0:3,0:3]))
sensor2ego_rotation = [ego2im_Q.w.astype(float), ego2im_Q.x.astype(float), ego2im_Q.y.astype(float), ego2im_Q.z.astype(float)]
cam_infos['sensor2ego_rotation'] = sensor2ego_rotation
cam_infos['sensor2ego_translation'] = ego2img[0:3, -1]
# ego2img_k = np.linalg.inv(view[:3, :3])
# ego2img_k@temp
# cam_infos['sensor2ego_translation']
# Rm = R.from_quat(sensor2ego_rotation)
...
for ann, label in zip(ann_infos[0], ann_infos[1]):
    ...
    box = Box(center=xyz, size=wlh, orientation=Quaternion(rot_axis+0.6), label=np.nan, score=np.nan, \
    ...
    # temp_matrix = np.array([[1, 0, 0],
    #                        [0, -1, 0],
    #                        [0, 0, 1]])


    # lidar_to_camera_matrix1 = lidar_to_camera_matrix@temp_matrix
