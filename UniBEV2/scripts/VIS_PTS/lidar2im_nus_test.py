...
def point2img(information, index, view, intrins, lidar2lidarego, save_path,cams_name):
    ...
    cv2.imwrite(os.path.join(save_path, 'Nus'+information[index]['token']+'_DepthFusion'+view+'.jpg'), img_Depth)
    cv2.imwrite(
        os.path.join(save_path, 'Nus' + information[index]['token'] + '_EgeFusion' + view + '.jpg'), img_Ege)
    return depth_map
...
info_path = './data/nuscenes/bevdetv2-nuscenes_infos_val.pkl'
info_path_test = '/mnt/cfs/algorithm/hao.lu/Code/UniBEV/work_dirs/bevdet-r50-cbgs-NUS2X-intri-nobn/DA2nus.pkl'
dataset = pickle.load(open(info_path, 'rb'))
pred_results = pickle.load(open(info_path_test, 'rb'))
save_path = './scripts/VIS_PTS/test'
print('saving visualized result to %s' % save_path)
# breakpoint()

list = [dataset['infos'][i*110] for i in range(5)]
pred_results = [pred_results[i*110] for i in range(5)]
...
for cnt, infos in enumerate(list):
    ...
    gt_boxes = pred_results[cnt]['pts_bbox']['boxes_3d']
    # breakpoint()
    score = pred_results[cnt]['pts_bbox']['scores_3d']
    # gt_boxes = [np.array(np.array(kk) for kk in gt_boxes)]
    # breakpoint()
    ...
    corners_lidar_gt = gt_boxes.corners.numpy()
    mask = score > 0.3
    corners_lidar_gt = corners_lidar_gt[mask].reshape(-1, 3)
    ...
    cv2.imwrite(os.path.join(save_path, 'NUS%s.jpg' % infos['token']), img)
    ...
    print('lidar2ego_rotation', list[cnt]['lidar2ego_rotation'])
    print('lidar2ego_translation', list[cnt]['lidar2ego_translation'])
    print('lidar2lidarego', lidar2lidarego)
    cams_name = 'CAM_FRONT'
