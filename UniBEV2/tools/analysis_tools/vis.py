...
def main():
    ...
    for cnt, infos in enumerate(
        ...
        if len(pred_boxes) == 0:
            ...
            # corners_lidar = corners_global @ np.linalg.inv(l2g).T # add by xxl
            # corners_lidar = corners_lidar[:, :3]
            corners_lidar = corners_global[:, :3]
        pred_flag = np.ones((corners_lidar.shape[0] // 8, ), dtype=np.bool)

        if "score" in pred_res[0].keys():
            scores = [
            pred_res[rid]['score'] for rid in range(len(pred_res))
            ]
        else:
            scores = [
        ...
        # image view
        imgs = []
        # corners_image_kk = corners_lidar.copy()
        
        mask  = scores > args.vis_thred
        corners_lidar_img = corners_lidar.copy().reshape(-1, 8, 3)[mask].reshape(-1, 3)
        for view in views:
            img = cv2.imread(infos['cams'][view]['data_path'])
            # draw instances
            
            corners_img, valid = ego2img(corners_lidar_img, infos['cams'][view])
            valid = np.logical_and(
                valid,
        ...
        corners_lidar = corners_lidar[:, :, [1, 0, 2]]
        corners_lidar[:, :, 0] = -corners_lidar[:, :, 0] # add by xxl
