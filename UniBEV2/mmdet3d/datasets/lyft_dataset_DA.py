...
class LyftDataset_DA(LyftDataset_my):
    def __init__(self,
        ...
        if Target_CLASSES==None:
            self.Target_CLASSES = ['car', 'bicycle', 'pedestrian']
        else:
            self.Target_CLASSES = Target_CLASSES
        if Source_CLASSES==None:
            self.Source_CLASSES = ['car', 'bicycle', 'pedestrian']
        else:
            self.Source_CLASSES = Source_CLASSES
        if Test_CLASSES==None:
            self.Test_CLASSES = ['car', 'bicycle', 'pedestrian']
        else:
            self.Test_CLASSES = Test_CLASSES
    ...
    def _format_bbox(self, results, metas_pre, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            for i, box in enumerate(boxes):
                if len(mapped_class_names)<(box.label-1):
                    continue
                ...
                if mask[0]==True:
                    ...
                    if sample_annotations[1][index] < len(self.Target_CLASSES):
                        ...
                        if class_name in self.Test_CLASSES:
                            size_temp = box[0].wlh.tolist()
                            annotation = dict(
                                sample_token=sample_token,
                                translation=box[0].center.tolist(),
                                size=[abs(size_temp[0]), abs(size_temp[1]), abs(size_temp[2])],
...
@DATASETS.register_module(force=True)
class SingleDataset_CBGS(LyftDataset_my):
    def __init__(self,
                 ann_file,
                 pipeline=None,
                 data_root=None,
        ...
        self.Source_CLASSES = Source_CLASSES
        self.Test_CLASSES = Test_CLASSES
        super(SingleDataset_CBGS, self).__init__(
            data_root=data_root,
            ann_file=ann_file,
            pipeline=pipeline,
            classes=classes,
        ...
        #     print(sample)
        self.data_infos = self.CBGS(self.data_infos)

    def CBGS(self, data_infos):
        self.label_class = [0, 0, 0]
        for sample in data_infos:
            gt_boxes, gt_labels = sample['ann_infos']
            for label in gt_labels:
                self.label_class[label]=self.label_class[label]+1
...
@DATASETS.register_module(force=True)
class UniBEVDataset(LyftDataset_DA):
    def __init__(self,
                 ann_files,
                 pipeline=None,
                 data_root=None,
        ...
        # load annotations
        if isinstance(self.ann_files, list):
            self.data_infos = list()
            for ann_file in self.ann_files:
                if hasattr(self.file_client, 'get_local_path'):
                    with self.file_client.get_local_path(ann_file) as local_path:
                        data_info = self.load_annotations(open(local_path, 'rb'))
                        if 'waymo' in self.ann_file or 'Waymo' in self.ann_file:
                    ...
                    data_info = self.load_annotations(ann_file)
                    if 'waymo' in self.ann_file or 'Waymo' in self.ann_file:
                        data_info = data_info[::5]
                    self.data_infos.append(data_info)
        else:
            if hasattr(self.file_client, 'get_local_path'):
                with self.file_client.get_local_path(self.ann_files) as local_path:
                ...
                warnings.warn(
                    ...
                    'might cause errors if the path is not a local path. '
                    'Please use MMCV>= 1.3.16 if you meet errors.')
                self.data_infos = self.load_annotations(self.ann_files)
                if 'waymo' in self.ann_file or 'Waymo' in self.ann_file:
                    self.data_infos = self.data_infos[::5]
        if isinstance(self.ann_files, list):
            self.data_infos = self.mark_Domain(self.data_infos)
            self.data_infos = self.mix_Domain(self.data_infos)
    ...
    def mix_Domain(self, data_infos):
        if isinstance(self.ann_files, list):
            all_data = []
            for data_info in data_infos:
                all_data = all_data + data_info
            return all_data
        else:
            return data_infos

    def mark_Domain(self, data_infos):
        # 如果是多个数据集就要打标注
        # 如果是单个数据集就不用打标注
        if isinstance(self.ann_files, list):
            label = 0
    ...
    def prepare_train_data(self, index):
        ...
        if 'Target_Domain' in input_dict.keys():
            if (str(self.domain_label) != input_dict['Domain_Marks']) or (input_dict is None):
                return None
        else:
            if (input_dict is None):
                return None
        self.domain_label = self.domain_label + 1
        if self.domain_label>=len(self.ann_files):
            self.domain_label=0
