...
class UniBEVDataset(LyftDataset_my):
    def __init__(self,
                 ann_files,
                 pipeline=None,
                 data_root=None,
            ...
            for ann_file in self.ann_files:
                if hasattr(self.file_client, 'get_local_path'):
                    with self.file_client.get_local_path(ann_file) as local_path:
                        self.data_infos.append(self.load_annotations(open(local_path, 'rb')))
                else:
                    warnings.warn(
                    ...
                    self.data_infos.append(self.load_annotations(ann_file))
        else:
            if hasattr(self.file_client, 'get_local_path'):
    ...
    def prepare_train_data(self, index):
        ...
        if 'Target_Domain' in input_dict.keys():
            if (self.domain_label == input_dict['Domain_Marks']) or (input_dict is None):
                return None
        else:
            if (input_dict is None):
                return None
        self.domain_label = input_dict['Domain_Marks']
        self.pre_pipeline(input_dict)
